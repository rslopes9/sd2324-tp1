...
public class JavaRepShorts {
	...
	public Result<Short> createShort(String shortId, String userId, long timeStamp) {
		Log.info(() -> format("createShort : userId = %s", userId));


			var blobsURLs = getAllBlobServerURI();
			var blobUrl = format("%s/%s/%s", blobsURLs.get(0), Blobs.NAME, shortId); 
			for (int i = 1; i < blobsURLs.size(); i++) {
				blobUrl = blobUrl + format("|%s/%s/%s", blobsURLs.get(i), Blobs.NAME, shortId);
			}
	...
	public Result<Void> deleteShort(String shortId) {
		Log.info(() -> format("deleteShort : shortId = %s", shortId));
	...
	public List<String> getAllBlobServerURI() {
        try {
            var servers = blobCountCache.get(BLOB_COUNT);
